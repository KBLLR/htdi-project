...
export default class TweakpaneManager {
  constructor(experience, opts = {}) {
    ...
    if (titleElement) {
      ...
      logoImg.src = '/cursor/logo.svg';
      logoImg.alt = 'Logo';
      logoImg.style.cssText = 'height: 1.5em; margin-right: 0.5em; vertical-align: middle;';
      titleElement.prepend(logoImg);
    }

    // canvas (to disable pointer events when expanded)
    this.canvas = experience.canvas ?? null;
    if (expanded) {
    ...
    // reapply PBR on registry changes
    const unregisterCharacters = this.experience.sceneRegistryApi?.onRegisterCategory?.(
      'characters',
      ({ name } = {}) => {
  ...
  createModelControls(name, modelRef) {
    ...
    model.traverse?.((child) => {
      if (child.isMesh && child.material) {
        ...
        mats.forEach((mat) => {
          if (!uniqueMaterials.has(mat.uuid)) uniqueMaterials.set(mat.uuid, mat);
        });
      }
    });
  ...
  addCharacters() {
    ...
    charactersFolder
      .addBinding(this.characterState, 'character', { label: 'Active', options })
      .on('change', (ev) => {
  ...
  /* ──────────────────────────────────────────────────────────────
     MATERIALS (simple -> copy)
  ────────────────────────────────────────────────────────────── */
  addMaterials() {
    const materialsFolder = this.pane.children
      .find((child) => child.title === 'Assets')
      ?.children.find((child) => child.title === 'Material Library');
    ...
    const toOptionsLocal = (names) => names.map((n) => ({ text: n, value: n }));
  ...
  applyTextureToMaterial(textureId, targetName) {
    ...
    const texture = this.loadTextureResource(`library:${textureEntry.id}`, textureEntry, {
      colorSpace: THREE.SRGBColorSpace,
    });
    if (!texture) {
      console.warn(`[textures] failed to load '${textureEntry.file ?? textureEntry.source}'`);
      return;
    }
  ...
  addParticles() {
    ...
    particlesFolder
      .addBinding(particleState, 'activeEmitter', { label: 'Emitter Type', options: emitterOptions })
      .on('change', (ev) => {
    ...
    this.addBinding(particlesFolder, {
      path: 'sceneRegistry.particles.main.ref.visible',
  ...
  updateParticleEmitterControls(container, emitter) {
    ...
    if (emitter.particleColor !== undefined) {
      controls.push(container.addBinding(emitter, 'particleColor', { label: 'Color', view: 'color' }));
    }

    if (emitter.material && 'opacity' in emitter.material) {
      controls.push(
        container.addBinding(emitter.material, 'opacity', { label: 'Opacity', min: 0, max: 1, step: 0.01 }),
      );
    }

    if (typeof emitter.velocityRange === 'number') {
      controls.push(
        container.addBinding(emitter, 'velocityRange', { label: 'Velocity', min: 0, max: 0.5, step: 0.01 }),
      );
    }
    ...
    if (typeof emitter.emissionRate === 'number') {
      controls.push(
        container.addBinding(emitter, 'emissionRate', { label: 'Emission Rate', min: 1, max: 100, step: 1 }),
      );
    }

    if (typeof emitter.areaSize === 'number') {
      controls.push(container.addBinding(emitter, 'areaSize', { label: 'Area Size', min: 1, max: 50, step: 1 }));
    }

    if (emitter.emitterShape) {
      controls.push(
        container.addBinding(emitter, 'emitterShape', {
          label: 'Shape',
          options: [
            { text: 'Box', value: 'box' },
    ...
    if (emitter.emitterPosition) {
      controls.push(
        container.addBinding(emitter.emitterPosition, 'x', {
          label: 'Pos X',
          min: -20,
          max: 20,
          step: 0.1,
      ...
      controls.push(
        container.addBinding(emitter.emitterPosition, 'y', {
          label: 'Pos Y',
          min: -20,
          max: 20,
          step: 0.1,
      ...
      controls.push(
        container.addBinding(emitter.emitterPosition, 'z', {
          label: 'Pos Z',
          min: -20,
          max: 20,
          step: 0.1,
    ...
    if (atlasFrames && typeof atlasFrames === 'object') {
      const textureNames = Object.keys(atlasFrames).map((key) => key.replace('public/particles/', ''));
      textureOptions = textureNames.map((name) => ({ text: name, value: name }));
    } else {
      textureOptions = [{ text: '(no atlas)', value: '__none__' }];
    }

    if (textureOptions.length) {
      const texBinding = container.addBinding(emitter, 'textureName', {
        label: 'Particle Texture',
        options: textureOptions,
      });
      texBinding.on('change', (ev) => {
        if (ev.value === '__none__') return;
        if (typeof emitter.updateParticleTextureUVs === 'function') {
          emitter.updateParticleTextureUVs(ev.value);
        }
      });
      controls.push(texBinding);
    }
  ...
  addEnvironmentControls() {
    ...
    fogFolder
      .addBinding(fogState, 'density', { label: 'Density', min: 0, max: 0.2, step: 0.001 })
      .on('change', (ev) => {
  ...
  addLightingControls() {
    ...
    lightsFolder
      .addBinding(goboBindingState, 'gobo', { label: 'Gobo', options: goboOptions })
      .on('change', (ev) => {
  ...
  updateActivePbrMaterial(partial) {
    if (partial && typeof partial === 'object') {
      if (Object.prototype.hasOwnProperty.call(partial, 'iridescenceThicknessMin')) {
        if (
          partial.iridescenceThicknessMin >
          (partial.iridescenceThicknessMax ?? this.pbrMaterialState.iridescenceThicknessMax)
        ) {
      ...
      if (Object.prototype.hasOwnProperty.call(partial, 'iridescenceThicknessMax')) {
        if (
          (partial.iridescenceThicknessMin ?? this.pbrMaterialState.iridescenceThicknessMin) >
          partial.iridescenceThicknessMax
        ) {
  ...
  addVideoControls() {
    ...
    const updateVideoState = () => {
      ...
      statusMonitor.value = videoState.status;
    };
  ...
  handleSceneApplied(event) {
    ...
    const fog = this.normaliseFogState(
      this.experience.getFogState?.() ?? sceneState.fog ?? this.fogState,
    );
    Object.assign(this.fogState, fog);

    const spotlight = this.normaliseSpotlightState(
      this.experience.getSpotlightState?.() ?? sceneState.lighting?.spotlight ?? this.spotlightState,
    );
  ...
  buildKidConfigFromVariant(variant, material, fallback = {}) {
    ...
    const color = material?.color?.isColor ? formatColor(material.color) : fallback?.color ?? '#ffffff';
  ...
  loadTextureResource(cacheKey, fileEntry, { colorSpace } = {}) {
    ...
    const fileName = fileEntry.file ?? fileEntry.source;
    if (!fileName) {
      console.warn('[textures] file entry missing "file" or "source"', fileEntry);
      return null;
    }
    const options = {
      path: fileEntry.path ?? '',
    ...
    try {
      return loadTextureAsset(`tweakpane:${cacheKey}`, fileName, options);
    } catch (error) {
      console.warn(`[textures] failed to load resource '${fileName}'`, error);
      return null;
    }
  }
