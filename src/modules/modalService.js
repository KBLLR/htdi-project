...
export function initModalService({ focusCameraOnModal, resetCameraFromModal, registry } = {}) {
  ...
  const offOpen = bus.on('modal:open', (p) => openModal(p?.id, p));
  const offClose = bus.on('modal:close', closeModal);
